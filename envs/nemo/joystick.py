...
from envs.nemo import base as base
from envs.nemo.base import step
...
class Joystick(base.NEMOEnv):
  ...
  def _post_init(self) -> None:
    ...
    for side in ["l", "r"]:
      knee_indices.append(
          self._mj_model.joint(f"{side}_knee").qposadr - 7
      )
  ...
  def reset(self, rng: jax.Array) -> mjx_env.State:
    ...
    rng, key = jax.random.split(rng)
    qpos = qpos.at[7:].set(
        qpos[7:] * jax.random.uniform(key, (self.ids["ctrl_num"],), minval=0.5, maxval=1.5)
    )
